...
<x-app-layout title="Clubes">
    <x-slot name="header">
        <div class="flex items-center justify-between gap-4">
            <div>
                <h2 class="text-2xl font-bold text-slate-800">Clubes</h2>
                <p class="text-sm text-slate-500">Gerencie os clubes criados pelos usuários.</p>
            </div>
        </div>
    </x-slot>

    <div
        id="clubes-filters-modal"
        ...
        <div class="w-full max-w-2xl rounded-2xl bg-white p-6 shadow-xl">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-sm text-slate-500">Refine a lista por liga.</p>
                </div>
                <button
                    type="button"
                    data-close-clubes-filters
            ...
            <form
                id="clubes-filters-form"
                action="{{ route('admin.clubes.index') }}"
                method="GET"
            ...
            >
                ...
                <div>
                    <label class="text-sm font-semibold text-slate-700" for="clubes-filter-liga">Liga</label>
                    <select
                        id="clubes-filter-liga"
                        name="liga_id"
                ...
                <div class="flex flex-wrap items-center justify-between gap-3 pt-4">
                    <a href="{{ route('admin.clubes.index') }}" class="text-sm font-semibold text-slate-500 transition hover:text-slate-700">
                        Limpar filtros
                    </a>
        ...
        <div class="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
            <div class="border-b border-slate-100 px-6 py-5">
                <div class="flex flex-wrap items-start justify-between gap-4">
                    <div>
                        <h3 class="text-lg font-semibold text-slate-900">Clubes cadastrados</h3>
                        <p class="text-sm text-slate-500">Nome, dono, liga e saldo.</p>
                    </div>
                    <div class="flex flex-wrap items-center gap-3">
                        <form
                            id="clubes-search-form"
                            action="{{ route('admin.clubes.index') }}"
                            method="GET"
                            class="flex flex-1 min-w-[260px] items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2 shadow-sm"
                        >
                            <label class="sr-only" for="clubes-search-input">Buscar clubes</label>
                            <input
                                id="clubes-search-input"
                                type="search"
                                name="search"
                                value="{{ $filters['search'] }}"
                                placeholder="Buscar clube ou usuário"
                                        ...
                                        <div class="flex items-center gap-3">
                                            ...
                                            @else
                                                <span class="inline-flex h-10 w-10 items-center justify-center rounded-xl bg-slate-100 text-xs font-semibold uppercase text-slate-500">
                                                    {{ strtoupper(substr($clube->nome, 0, 2)) }}
                                                </span>
                                            @endif
                                            <div>
                                                <div class="text-sm font-semibold text-slate-900">{{ $clube->nome }}</div>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="px-4 py-4">
                                        @if($clube->user)
                                            <div class="font-semibold text-slate-900">{{ $clube->user->nickname ?? $clube->user->name }}</div>
                                            <div class="text-xs text-slate-500">ID {{ $clube->user->id }} · {{ $clube->user->name }}</div>
                                        @else
                                            <span class="text-xs text-slate-400">Sem usuário</span>
                                        @endif
                                    </td>
                                    <td class="px-4 py-4 text-slate-600">{{ $clube->liga?->nome }}</td>
                                    <td class="px-4 py-4 text-right font-semibold text-slate-900">
                                        R$ {{ number_format($clube->financeiro?->saldo ?? 0, 0, ',', '.') }}
                                    </td>
                                    <td class="px-4 py-4 text-right">
                                        <div class="flex flex-wrap justify-end gap-2">
                                            <a
                                                href="{{ route('admin.clubes.edit', $clube) }}{{ $listingQuery }}"
                                            ...
                                            <form action="{{ route('admin.clubes.destroy', $clube) }}{{ $listingQuery }}" method="POST" class="inline">
                                                @csrf
                                                @method('DELETE')
                                                <button
                                                    type="submit"
    ...
    <script>
        (function () {
            const modal = document.getElementById('clubes-filters-modal');
            if (! modal) {
                return;
            }

            const openButton = document.querySelector('[data-open-clubes-filters]');
            const closeButtons = modal.querySelectorAll('[data-close-clubes-filters]');
            ...
            document.querySelectorAll('[data-confirm-delete]').forEach((button) => {
                button.addEventListener('click', (event) => {
                    if (! window.confirm('Tem certeza que deseja excluir este clube e todos os dados relacionados? Esta ação não pode ser desfeita.')) {
