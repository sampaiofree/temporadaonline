...
@php
    $nomeAtual = old('nome', $geracao->nome ?? '');
@endphp
...
<form action="{{ $action }}" method="POST" class="space-y-6">
    ...
    <div>
        <label for="nome" class="block text-sm font-semibold text-slate-700">Nome da geração</label>
        <input
            type="text"
            id="nome"
            name="nome"
    ...
    <div class="flex items-center justify-between gap-3 pt-6">
        <a href="{{ route('admin.geracoes.index') }}" class="text-sm font-semibold text-slate-600 hover:text-slate-900">Voltar para a lista</a>
        <button
            type="submit"
